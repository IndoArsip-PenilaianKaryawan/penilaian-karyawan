...
<div class="px-4 py-8 xl:ml-80 bg-[#F5F6F7] min-h-screen">
    <div class="flex flex-col md:flex-row md:items-center mb-4">
        <h1 class="text-xl md:text-3xl font-semibold">Halaman Kompetensi</h1>
        <a class="bg-[#9F2D2D] text-white md:p-4 p-2 w-fit rounded-2xl text-sm md:ml-4" href="/kompetensi/create"> <i class="fas fa-plus pr-2"></i> Tambah Kompetensi</a>
    </div>
    ...
    <table border="1" id="kompetensiTable">
        <thead>
            <tr>
                <th class="text-xs md:text-sm">Nama Kompetensi</th>
                <th class="text-xs md:text-sm">Deskripsi</th>
        ...
        <tbody>
            ...
            <tr>
                <td class="text-xs md:text-sm">{{ $item->nama_kompetensi }}</td>
                <td class="text-xs md:text-sm">{{ $item->deskripsi }}</td>
    ...
    deleteButtons.forEach(function(btn) {
        btn.onclick = function() {
            ...
            confirmBtn.onclick = function() {
                var form = document.querySelector("form[action='{{ route('kompetensi.destroy', '') }}/" + kompetensiId + "']");
                if (form) {
                    form.submit();
                }
            };
